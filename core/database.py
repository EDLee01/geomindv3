...
def init_database():
    ...
    # Projects 表 - 类似 Claude Projects 功能
    cursor.execute("""
    ...
    # 添加 project_id 到 threads 表（如果不存在）
    try:
        cursor.execute("ALTER TABLE threads ADD COLUMN project_id TEXT")
...
# ============================================================
# Projects 管理（类似 Claude Projects）
# ============================================================
...
def create_project(
    ...
    if is_default:
        cursor.execute(
            "UPDATE projects SET is_default = 0 WHERE user_id = ?",
            (user_id,)
        )

    cursor.execute(
        """INSERT INTO projects (id, user_id, name, description, instructions, icon, color, is_default)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        (project_id, user_id, name, description, instructions, icon, color, 1 if is_default else 0)
    )
    conn.commit()
    conn.close()
...
def get_project(project_id: str) -> Optional[Dict]:
    ...
    cursor.execute(
        """SELECT id, user_id, name, description, instructions, icon, color, is_default, created_at, updated_at
...
def get_default_project(user_id: int) -> Optional[Dict]:
    """获取用户的默认项目"""
    conn = _get_connection()
    cursor = conn.cursor()
...
def delete_project(project_id: str):
    ...
    # 将关联的 threads 的 project_id 设为 NULL
    cursor.execute("UPDATE threads SET project_id = NULL WHERE project_id = ?", (project_id,))
...
def get_project_threads(project_id: str, limit: int = 50) -> List[Dict]:
    """获取项目的所有对话"""
    conn = _get_connection()
    cursor = conn.cursor()
...
def set_thread_project(thread_id: str, project_id: str):
    """将对话关联到项目"""
    conn = _get_connection()
    cursor = conn.cursor()
...
def add_project_file(
    ...
    cursor.execute(
        """INSERT INTO project_files (id, project_id, filename, file_type, file_size, content_text, embedding_id)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (file_id, project_id, filename, file_type, file_size, content_text, embedding_id)
    )
...
def get_project_files(project_id: str) -> List[Dict]:
    ...
    cursor.execute(
        """SELECT id, project_id, filename, file_type, file_size, content_text, embedding_id, created_at
           FROM project_files
           WHERE project_id = ?
...
def get_project_file(file_id: str) -> Optional[Dict]:
    """获取单个文件信息"""
    conn = _get_connection()
    cursor = conn.cursor()
...
def delete_project_file(file_id: str):
    ...
    # 获取 project_id 以更新项目时间
    cursor.execute("SELECT project_id FROM project_files WHERE id = ?", (file_id,))
    row = cursor.fetchone()
    ...
    if row:
        cursor.execute(
            "UPDATE projects SET updated_at = ? WHERE id = ?",
            (datetime.now().isoformat(), row["project_id"])
        )
...
def get_project_knowledge_context(project_id: str) -> str:
    """获取项目的知识库上下文（所有文件内容合并）"""
    ...
    for f in files:
        if f["content_text"]:
            context_parts.append(f"[文件: {f['filename']}]\n{f['content_text']}\n")
